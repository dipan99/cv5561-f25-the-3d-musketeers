...
OUTPUT_DIR="output_infer_Office2_vggt_part"
DATASETS=(
    examples
)
...
run_on_gpu() {
    ...
    CUDA_VISIBLE_DEVICES=${GPU_ID} python -W ignore ./init_geo_vggt.py \
    -s ${SOURCE_PATH} \
    -m ${MODEL_PATH} \
    --n_views ${N_VIEW} \
    --infer_video \
    > ${MODEL_PATH}/01_init_geo.log 2>&1
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Co-visible Global Geometry Initialization completed. Log saved in ${MODEL_PATH}/01_init_geo_vggt.log"
